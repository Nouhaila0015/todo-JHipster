...
describe('Todo Management Detail Component', () => {
      ...
      providers: [
        provideRouter(
          [
            {
              path: '**',
              loadComponent: () => import('./todo-detail.component').then(m => m.TodoDetailComponent),
              resolve: { todo: () => of({ id: 28437 }) },
            },
          ],
  ...
  describe('OnInit', () => {
    it('Should load todo on init', async () => {
      ...
      // THEN
      expect(instance.todo()).toEqual(expect.objectContaining({ id: 28437 }));
    });
  });
