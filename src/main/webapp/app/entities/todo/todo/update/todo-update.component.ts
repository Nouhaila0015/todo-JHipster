...
import { TodoFormGroup, TodoFormService } from './todo-form.service';
...
export class TodoUpdateComponent implements OnInit {
  ...
  protected updateForm(todo: ITodo): void {
    ...
    this.usersSharedCollection = this.userService.addUserToCollectionIfMissing<IUser>(this.usersSharedCollection, todo.user);
  }
  ...
  protected loadRelationshipsOptions(): void {
    this.userService
      ...
      .pipe(map((users: IUser[]) => this.userService.addUserToCollectionIfMissing<IUser>(users, this.todo?.user)))
